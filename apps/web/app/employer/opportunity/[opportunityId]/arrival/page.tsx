    ...
    <EmployerChrome>
      <div className={shared.page}>
        <header className={shared.header}>
          <Link href="/employer" className={shared.back}>
            بازگشت به کارها
          </Link>
          <p className="type-label">
            <Latin>E-06</Latin> · برش محدود ورود به شیفت
          </p>
          <h1 className="type-display">کد حضور در شیفت نمایشی</h1>
          <p className="type-body">
            این کد فقط در همین زبانهٔ نمایش کار می‌کند. ایجاد کد یا جلو بردن ساعت، حضور واقعی کارگر
            را ثابت نمی‌کند و پیامک یا موقعیت مکانی واقعی در کار نیست.
          </p>
        </header>
        {!restored ? (
          <p className="type-body">در حال خواندن وضعیت…</p>
        ) : !allowed || record === undefined || engagement === undefined ? (
          ...
          <section className={shared.section} data-testid="employer-arrival-ready">
              ...
              <>
                {code === undefined ? (
                  <button
                    ...
                    onClick={() => act(() => issueArrivalCode(opportunityId))}
                    data-testid="employer-issue-code"
                  >
                    صدور کد نمایشی
                  </button>
                ) : (
                  <p className="type-body" data-testid="employer-issued-code">
                    کد کوتاه برای اعلام حضوری در همین سناریوی ساختگی:{' '}
                    <Latin>{code.code.value}</Latin>
                  </p>
                )}
                <p className="type-detail">
                  تأیید کد <TruthChip level="FUNCTIONAL" href="/truth#truth-row-10" /> · سناریوی
                  زمان و حضور <TruthChip level="SIMULATED" href="/truth#truth-row-10" />
                </p>
                {!shifted ? (
                  <button
                    type="button"
                    className={controls.secondaryAction}
                    onClick={() => act(() => advanceScenarioClock(opportunityId))}
                    data-testid="employer-advance-clock"
                  >
                    جلو بردن ساعتِ نمایش تا آغاز شیفت
                  </button>
                ) : (
                  <p className="type-body" data-testid="employer-clock-advanced">
                    ساعت همین سناریوی ساختگی به زمان شروع شیفت رسیده است.{' '}
                    <TruthChip level="SIMULATED" href="/truth#truth-row-10" />
                  </p>
                )}
                <p className="type-detail">
                  در نقش کارگر، W-05 را باز کنید و کد را وارد کنید. هیچ شروع کاری بدون اقدام کارگر
                  ثبت نمی‌شود.
                </p>
              </>
            ) : (
              <p className="type-body" data-testid="employer-arrived">
                ورود نمایشی با کد ثبت شده است؛ کد دیگر قابل استفاده نیست.{' '}
                <TruthChip level="SIMULATED" href="/truth#truth-row-10" />
              </p>
            )}
            {error === null ? null : (
            ...
            <p className="type-detail">
              ثبت اصلاحیهٔ شیفت در این برش ساخته نشده است — <Latin>PLANNED</Latin>.
            </p>
          </section>
        )}
      </div>
