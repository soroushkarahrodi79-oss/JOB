...
| --- | --- | --- | --- | --- |
| 1 | Worker profile and skills | `FUNCTIONAL` | `FUNCTIONAL` | Domain-owned; no external dependency. |
...
| 5 | Opportunity creation and publication | `FUNCTIONAL` | `FUNCTIONAL` | |
...
| 9 | Engagement classification | `FUNCTIONAL` | `FUNCTIONAL` | The *mechanism* is functional. Its legal correctness is unvalidated — see row 18. |
| 10 | Proof of Work capture | `SIMULATED` | `SIMULATED` | Attestation logic is real; any device/location attestation is simulated. |
| 11 | Payment orchestration | `SIMULATED` | `SIMULATED` | Ledger and state machine are real; settlement is simulated. No funds move. |
...
| 13 | Dispute handling | `FUNCTIONAL` | `FUNCTIONAL` | Case state machine only. Outcomes are recorded, not enforced. |
| 14 | Reputation — worker **and** employer | `FUNCTIONAL` | `FUNCTIONAL` | Event-derived projections cover both sides; no composite score. |
| 15 | Preferred Crew / rehire | `FUNCTIONAL` | `PLANNED` | |
| 16 | Messaging / SMS notification | `MOCK` | `MOCK` | Deterministic mock adapter; no message leaves the system. |
| 17 | Authentication | `SIMULATED` | `SIMULATED` | Deterministic simulated session outcome only; no credential/OTP channel exists. |
...
| 21 | Demo scaffolding — actor switch, demo reset | `FUNCTIONAL` | `FUNCTIONAL` | Real logic, and it exists only in the prototype. Must declare itself a demo mechanism, not a product feature. Actor-switch mechanism built in SH-01; the actor home destinations (W-01, E-01, OPS-01) remain `PLANNED`. |
| 22 | In-product truth ledger | `FUNCTIONAL` | `FUNCTIONAL` | Renders this matrix inside the prototype and is reachable from every truth label. Required by the presentation rules in `product/demo-scenarios.md`. Built as SH-02; a per-row anchor exists for truth-chip deep-links. |
